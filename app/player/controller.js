...
module.exports = {
  ...
  detailPage: async (req, res) => {
    try {
      const { id } = req.params;
      const detail = await Voucher.findOne({ _id: id }).populate("category").populate("nominals").populate("user", "_id name phoneNumber");
    ...
    try {
      ...
      if (req.file) {
        ...
        src.on("end", async () => {
          ...
          player = await Player.findOneAndUpdate({ _id: req.player._id }, { ...payload, avatar: filename }, { new: true, runValidators: true });
